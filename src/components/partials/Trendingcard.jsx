...
function Trendingcard({trend,category}) {
  ...
  return (
    <div className="md:h-[32%]  w-full flex overflow-hidden overflow-x-auto gap-4 pb-4">
     

      {(trend.map((elem,index)=>{
        return  <Link key={index} to={`/${elem.media_type || category}/details/${elem.id}`} className="w-[100vh] ">
      
        <div  className="h-full w-[32vh] rounded-lg flex flex-col shrink-0 bg-zinc-700 overflow-hidden overflow-y-auto">
        <div className=" h-[55%]">
        ...
        <div className="pl-4 h-[45%]">
          <h1 className="font-bold text-2xl">{elem.original_title || elem.name||elem.title}</h1>
          <p className="text-xs md:leading-[1.5vh]  text-zinc-400 pt-3">{elem.overview.slice(0,100)}... <span className="text-sky-400">more</span></p>
        </div>
      </div>
      </Link>
    }))}
